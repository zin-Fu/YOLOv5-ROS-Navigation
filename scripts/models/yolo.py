...
class Model(nn.Module):
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        ...
        for f, s in zip(m.f, m.stride):  #  from
            ...
            # b[:, 4] += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image) # FZL
            # b[:, 5:] += math.log(0.6 / (m.nc - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
            # mi.bias = torch.nn.Parameter(b.view(-1), requires_grad=True)
            b_clone = b.clone()
            b_clone[:, 4] += math.log(8 / (640 / s) ** 2)  # obj (8 objects per 640 image)
            b_clone[:, 5:] += math.log(0.6 / (m.nc - 0.99)) if cf is None else torch.log(cf / cf.sum())  # cls
            mi.bias = torch.nn.Parameter(b_clone.view(-1), requires_grad=True)
